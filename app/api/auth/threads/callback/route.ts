...
const REDIRECT_URI = 'https://u-one-shot.vercel.app/api/auth/threads/callback';
...
export async function GET(request: Request) {
  ...
  if (oauthError) {
    return NextResponse.redirect(new URL(`/dashboard/publish?error=${encodeURIComponent(oauthError)}`, request.url));
  }
  ...
  if (!appId || !appSecret) {
    return NextResponse.redirect(new URL('/dashboard/publish?error=server_not_configured', request.url));
  }
  ...
  try {
    ...
    const { error } = await supabase.from('uos_threads_accounts').upsert(
      {
        user_id: user.id,
    ...
    return NextResponse.redirect(new URL('/dashboard/publish?connected=threads', request.url));
  } catch (err) {
